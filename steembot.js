...
class SteemBot extends EventEmitter{
  static fromFile(filepath){
    const mgr = new SteemBot(JSON.parse(fs.readFileSync(filepath).toString().trim()))
...
module.exports = SteemBot

if (require.main === module){
  const filepath = process.argv.pop()
  const mgr = SteemBot.fromFile(filepath)
  mgr.init().then(res => {
